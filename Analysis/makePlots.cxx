...
int makePlots::Loop1(int chip, int channel, int SCA){
    ...
    //------------------------
        ...
        for(int hit = 0; hit < nhits1 ; ++hit){
            ...
            if(H.chip == chip && H.ch == channel){
                if(TS[SCA] <9 ){ h1->Fill(H.SCA_hg[SCA]); }
                }
            }
        }
...
void makePlots::Loop2(int chip, int channel, int SCA){
...
void makePlots::Loop3(float Pedestal_mean, int chip, int channel, int SCA){
    //--------------------Find it is Pedestal run or charge run--------------------//
    Init();
    float Substrate_mean=0;
    float Substrate_error=0;
    ...
    TH1F *h2 = new TH1F("h2","",100,-50,50); //("title","",slice,star,end)
        ...
        for(int hit = 0; hit < nhits2 ; ++hit){
            ...
            if(!H.CCorNC) continue; // remove unconnected channel
            if(H.chip == chip && H.ch == channel){
                    if(TS[SCA]<9){h2->Fill(H.SCA_hg[SCA]-Pedestal_mean);
                    }}}}
        h2->Fit(Full_Gaussian2,"R");
        Substrate_mean=Full_Gaussian2->GetParameter(1);
        Substrate_error=Full_Gaussian2->GetParameter(2);
        TLegend *leg = new TLegend(0.1,0.7,0.4,0.9);
        leg->AddEntry(Full_Gaussian2,Form("Mean:%f",Substrate_mean),"l");
        leg->AddEntry(Full_Gaussian2,Form("Error:%f",Substrate_error),"l");
        leg->SetFillColor(0);
        leg->SetFillStyle(0);
        leg->SetTextSize(0.05);
        leg->SetTextFont(22);
