...
for k in range(0,2):
    ...
    #-------------------------------------------setting the starting point
        ...
        #-------------------------------------------starting analyze the ROC curve
                ...
                for p in range(1,numbin):
                    if (h1.Integral(1,p)<(h1.Integral(1,numbin)/2)<h1.Integral(1,p+1)):
                        break
                print str((p+1)*5)
                Central_energy=(p+1)*5
                print 'Central_energy:'+str(Central_energy)
                print 'Files:'+str(files_array[i])
                #===============Starting point==============#
                Left_bin=(p+1)#bin
                Right_bin=(p+2)#bin
                xarray.append(h1.Integral(Left_bin,Right_bin)/a)
                yarray.append(1/((h2.Integral(Left_bin,Right_bin))/b))
                print str(h1.Integral(Left_bin,Right_bin)/a)
                print str(1/(h2.Integral(Left_bin,Right_bin)/b))
                Latest_Window=np.array([[Left_bin,Right_bin]])
                #===========================================#
                for run_number in range(numbin):
                    print 'run_number:'+str(run_number)
                    if(Left_bin==1 and Right_bin!=numbin):
                        xarray.append(h1.Integral(Left_bin,Right_bin+1)/a)
                        yarray.append(1/((h2.Integral(Left_bin,Right_bin+1))/b))
                        print str(h1.Integral(Left_bin,Right_bin+1)/a)
                        print str(1/(h2.Integral(Left_bin,Right_bin+1)/b))
                        Left_bin=Left_bin
                        Right_bin=Right_bin+1
                        Window=np.array([[Left_bin,Right_bin]])
                        Latest_Window=np.concatenate((Latest_Window, Window))
                    elif(Right_bin==numbin and Left_bin!=1):
                        xarray.append(h1.Integral(Left_bin-1,Right_bin)/a)
                        yarray.append(1/((h2.Integral(Left_bin-1,Right_bin))/b))
                        print str(h1.Integral(Left_bin-1,Right_bin)/a)
                        print str(1/(h2.Integral(Left_bin-1,Right_bin)/b))
                        Left_bin=Left_bin-1
                        Right_bin=Right_bin
                        Window=np.array([[Left_bin,Right_bin]])
                        Latest_Window=np.concatenate((Latest_Window, Window))
                    elif(Left_bin!=1 and Right_bin!=numbin):
                        xarray.append(h1.Integral(Left_bin-1,Right_bin+1)/a)
                        yarray.append(1/((h2.Integral(Left_bin-1,Right_bin+1))/b))
                        print str(h1.Integral(Left_bin-1,Right_bin+1)/a)
                        print str(1/(h2.Integral(Left_bin+1,Right_bin+1)/b))
                        Left_bin=Left_bin-1
                        Right_bin=Right_bin+1
                        Window=np.array([[Left_bin,Right_bin]])
                        Latest_Window=np.concatenate((Latest_Window, Window))
                        ...
                        Window=np.array([[Left_bin,Right_bin]])
                        Latest_Window=np.concatenate((Latest_Window, Window))
                        run_number_use=run_number
                        break
                n=run_number_use+2
                print 'n:'+str(n)
                print 'Central_energy:'+str(Central_energy)
                print 'numbin:'+str(numbin)
        ...
        #leg.Draw()
        #c.Draw()
                f=TFile("A_Cluster_"+str(files_array[i])+"_"+variable[k]+"_"+str(energy_array[1][m])+"tev_eff_1_central_fix_at_Median_bin_"+str(Central_energy)+"GeV_"+str(signal_we_want[l])+"_qq_log_no_UOF.root","RECREATE")
                gr.Write()
