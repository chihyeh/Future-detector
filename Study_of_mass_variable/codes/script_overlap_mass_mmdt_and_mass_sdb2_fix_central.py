    ...
    for d in range(0,2):
        for i in range(0,4):
            for j in range(0,9):
                if(variable_array[k]=="mass_sdb2"):
                    if (signal_we_want[d]=="tt"):
                        if(energy_array[1][i]==5):
                            central_energy=160
                        if(energy_array[1][i]==10):
                                central_energy=200
                        if(energy_array[1][i]==20):
                            central_energy=280
                        if(energy_array[1][i]==40):
                            central_energy=310
                if(variable_array[k]=="mass_sdb2"):
            ...
            #-
                ...
                f1= ROOT.TFile.Open("/Users/ms08962476/MannWhitneyU/A_Cluster_010_"+variable_array[k]+"_"+str(energy_array[1][i])+"tev_eff_1_central_fix_at_"+str(central_energy+5*j)+"GeV_"+signal_we_want[d]+"_qq.root",'r')
                G1=f1.Get("Graph")
                G1.SetLineStyle(1)
                ...
                f2 = ROOT.TFile.Open("/Users/ms08962476/MannWhitneyU/A_Cluster_009_"+variable_array[k]+"_"+str(energy_array[1][i])+"tev_eff_1_central_fix_at_"+str(central_energy+5*j)+"GeV_"+signal_we_want[d]+"_qq.root",'r')
                G2=f2.Get("Graph")
                G2.SetLineStyle(7)
                ...
                f3 = ROOT.TFile.Open("/Users/ms08962476/MannWhitneyU/A_Cluster_012_"+variable_array[k]+"_"+str(energy_array[1][i])+"tev_eff_1_central_fix_at_"+str(central_energy+5*j)+"GeV_"+signal_we_want[d]+"_qq.root",'r')
                G3=f3.Get("Graph")
                G3.SetLineStyle(10)
...
#       c.Print("cluster_"+detector_size_array[i]+"_tau21_compare_different_beta_"+str(energy_array[1][j])+"tev.eps")
                c.Print("A_Cluster_"+variable_array[k]+"_"+str(energy_array[1][i])+"tev_eff_1_central_fix_at_"+str(central_energy+5*j)+"GeV_"+signal_we_want[d]+"_qq.eps")
